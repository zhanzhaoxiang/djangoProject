...
def creat_nodes(labels, nodes, graph):
    ...
    for i in range(len(nodes)):
        for j in range(len(nodes[i])):
            node = Node(labels[i], name=nodes[i][j],id=j)  # 节点标签，名字
            graph.create(node)
            print('nodecount',nodecount+1)
...
if __name__ == '__main__':
    ...
    graphs = rel_neo4j()
    print('图连接完成')
    (labelss, nodess) = find_nodes(datas)
    print('抽取节点完成')
    (combinationss, relationss) = find_relation(datas, labelss)
    print('抽取关系完成')
    creat_nodes(labelss, nodess, graphs)
    print('创建节点完成')
    creat_relation(datas, combinationss, labelss, graphs, relationss)
    print('创建关系完成')
